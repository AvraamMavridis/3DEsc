...
public class Guard3 : MonoBehaviour {
    ...
    void Update()
    {
        ...
        if (rigidbody.transform.position.z < -1 && state == States.MovingUp)
        {
        ...
        else if ((int)rigidbody.transform.position.z == 0 && state == States.MovingUp)
        {
            rigidbody.transform.Rotate(new Vector3(0, 180, 0));
            state = States.MovingDown;
        }
        else if (rigidbody.transform.position.z > -23 && state == States.MovingDown)
        {
            rigidbody.transform.position = new Vector3(rigidbody.transform.position.x, rigidbody.transform.position.y, rigidbody.transform.position.z - 0.05f);
        }
        else if ((int)rigidbody.transform.position.z == -23 && state == States.MovingDown)
        {
            rigidbody.transform.Rotate(new Vector3(0, 180, 0));
            state = States.MovingUp;
        }
